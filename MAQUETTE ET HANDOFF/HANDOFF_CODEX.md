...
**Version de référence :** ce fichier, à la racine du paquet. Toute autre copie de `HANDOFF_CODEX.md` trouvée ailleurs est obsolète.
...
**75 références maquettées.** L'inventaire ligne par ligne — fichier, cadre ou ancre, type de vue, entrée, sortie, formats, états, statut, reste à fournir — est dans `SCREENS_MANIFEST.csv`. **C'est la source de vérité de l'inventaire** ; cette section n'en donne que la structure.

> Le décompte de la version précédente de ce document (70) était erroné : il comptait un module qui n'a jamais été maquetté (C7 · Règles automatisées) et sous-comptait C2 et les paramètres. Détail des quatre écarts dans `DECISIONS.md` § 14.

| Bloc | Références | Écrans | Fichier de maquette |
|---|---|---|---|
| **A** · Site public | A1 → A8 | 8 | `Lot 7 - Site public.dc.html` · A1 remplacé par `Lot 7 - A1 Landing page v2.dc.html` |
| **B** · Mise en route Meta | B1 → B6 | 6 | `Lot 6 - Onboarding Business Brain et Parametres.dc.html` |
| **B** · Business Brain | B7 → B11 | 5 | idem |
| **C1** · Tableau de bord | C1.1, C1.2 | 2 | `Lot 1 - C1.1 Tableau de bord.dc.html` · `Lot 1 - C1.2 Selecteur de compte.dc.html` |
| **C2** · Agent d'optimisation | C2.1 → C2.5 | 5 | `Lot 1 - Partie 3 Agent d optimisation.dc.html` |
| **C3** · Gestionnaire de publicités | C3.1 → C3.3 | 3 | `Lot 2 - Tableau mobile et modele.dc.html` |
| **C4** · Constructeur de campagne | C4.1 → C4.9 | 9 | `Lot 3 - Etape 1 à 4` · états et déclinaisons : `Etape 5, 6, 7` |
| **C5** · Studio créatif | C5.1 → C5.7 | 7 | `Lot 4 - Studio et Analyse creative.dc.html` |
| **C6** · Analyse créative | C6.1 → C6.6 | 6 | idem · déclinaisons `Lot 4 - Tablette 768`, `Lot 4 - Mobile 375` |
| **C8** · Intelligence marché | C8.1 → C8.8 | 8 | `Lot 5 - Intelligence marche et Rapports.dc.html` |
| **C9** · Rapports | C9.1 → C9.5 | 5 | idem |
| **C10** · Business Brain en consultation | C10.1 → C10.3 | 3 | `Lot 6 - Onboarding Business Brain et Parametres.dc.html` |
| **C11** · Paramètres et facturation | C11.1 → C11.8 | 8 | idem |
| **C7** · Règles automatisées | — | **0** | **aucune maquette — module à concevoir** |

Hors numérotation, et nécessaires : `Lot 1 - Coquille applicative.dc.html` (3 largeurs de barre, 6 groupes, bandeaux système), le bloc « Composants communs » de `Lot 7 - Site public.dc.html`, et les pages « États » des lots 3, 5 et 6.

### Ce qui est réellement assemblé dans le prototype

17 des 75 références : **A1, A3, A4, A5, A6, A7** (gabarit), **B1 → B6, C1.1, C2.1, C2.2, C3.1, C3.2, C3.3**.
Les 58 autres existent en maquette validée et sont annoncées dans le prototype par une page de module qui dit explicitement : « Ce module n'est pas encore assemblé dans le prototype interactif ». **Un nom dans le menu ou un lien vers une maquette n'est pas un écran livré.**

### Trois liens structurants à implémenter réellement

1. Le **CPA cible** saisi en B9 est le seuil que l'agent compare en C2.1.
2. Le **Business Manager en lecture seule** choisi en B3 déclenche le bandeau « Mode consultation » dans toute l'application.
3. La **page sans Instagram** choisie en B5 grise les placements Instagram en C4.5 et C4.8.
...
5. **Pilotage** — Règles automatisées *(module non maquetté)*, Analyse marché, Rapports
...
Le prototype persiste son état dans `localStorage` sous la clé `lyads-proto-v2`. En production, remplacer par la base. Les champs et leur signification :
...
onb         index d'étape de la mise en route Meta (0..5) — Bienvenue, Meta,
            Business Manager, Comptes, Pages, Pixel. Le Business Brain (B7–B11)
            est un parcours distinct, non assemblé dans le prototype
...
Deux actions : **Valider et appliquer** / **Ignorer**. Plus la mention de réversibilité : toute action est annulable depuis l'historique pendant 7 jours.
...
5. **Le cœur** — agent C2.1 et C2.2, livre de crédits, historique annulable 7 jours
6. **Réglages de l'agent** — C2.5 (objectifs, seuils, application automatique désactivée par défaut). Le module Règles automatisées C7 n'est pas maquetté : à cadrer avant développement
...
4. **Règles automatisées** — module entier à concevoir : écrans, seuils par défaut, garde-fou anti-contradiction. Voir `DECISIONS.md` § 14.
...
| Fichier | Rôle |
|---|---|
| `HANDOFF_CODEX.md` | **handoff principal de référence** — 16 sections, inventaire des 75 références |
| `SCREENS_MANIFEST.csv` | une ligne par référence : fichier, cadre, type de vue, entrée, sortie, formats, états, statut, reste à fournir |
| `INTERACTIONS.md` | comportement attendu écran par écran, et les points ouverts |
| `DECISIONS.md` | divergences entre sources, décision retenue et sa source, ou « à trancher » |
...
| `README.md` | ouverture locale des maquettes et ordre de lecture |
| `maquettes/` | les 19 fichiers de maquette validés, avec `support.js` |
| `Prototype Lyads - autonome.html` | prototype en fichier unique, hors connexion |
