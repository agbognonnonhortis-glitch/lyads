...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname.endsWith("/health")) {
      ...
      const ready = /^\d+$/.test(cfg.appId) &&
        /^[a-f0-9]{32}$/i.test(cfg.appSecret);
    ...
    if (request.method === "GET" && url.pathname.endsWith("/callback")) {
      ...
      if (!/^\d+$/.test(identity.id) || identity.id !== verified.userId) {
      ...
      const { data: connection, error } = await db.rpc(
        "lyads_save_verified_meta_connection",
        {
      ...
      return Response.redirect(
        cfg.origin +
          ...
          (access.complete ? "connected" : "partial"),
        303,
      );
    }
    ...
    const code = error instanceof MetaFailure
      ? error.code
    ...
    return json(
      {
        error: {
          code,
          message: META_MESSAGES[code] ||
            META_MESSAGES.META_TEMPORARILY_UNAVAILABLE,
        },
      },
      503,
    );
