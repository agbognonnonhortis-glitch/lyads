begin;
-- Keep existing selections and explicit skip choices; do not reset user data.
alter table public.lyads_onboarding drop constraint lyads_onboarding_pages_required;
alter table public.lyads_onboarding add constraint lyads_onboarding_pages_choice
 check ((not pages_skipped or cardinality(page_ids)=0) and (current_step<4 or cardinality(page_ids)>0 or pages_skipped));
...
declare d public.lyads_onboarding; old public.lyads_onboarding; v jsonb; a uuid; p text; k text; section text; field text; content jsonb; product jsonb; allowed text[]; begin
 ...
 if changes?'pages_skipped' then
  if jsonb_typeof(changes->'pages_skipped') is distinct from 'boolean' then raise exception 'Invalid page choice' using errcode='22023';end if;
  d.pages_skipped:=(changes->>'pages_skipped')::boolean;
 elsif changes?'page_ids' and cardinality(d.page_ids)>0 then d.pages_skipped:=false;end if;
 if d.pages_skipped and cardinality(d.page_ids)>0 then raise exception 'Select pages or explicitly skip' using errcode='22023';end if;
 ...
 -- Adding optional resources after signup must not restart onboarding or billing.
 if old.completed_at is not null and d.connection_id is not distinct from old.connection_id
  and not exists(select 1 from jsonb_object_keys(changes) key where key not in ('page_ids','pages_skipped','pixels','pixels_skipped','current_step')) then
  d.completed_at:=old.completed_at;d.reviewed_at:=old.reviewed_at;d.current_step:=10;
 end if;
 ...
 if d.current_step>=4 and cardinality(d.page_ids)=0 and not d.pages_skipped then raise exception 'Select a Facebook page' using errcode='22023';end if;
