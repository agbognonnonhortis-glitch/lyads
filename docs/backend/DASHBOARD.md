...
`lyads_dashboard_scope` valide compte → campagne → ensemble avec RLS et renvoie les listes complètes importées. Le menu distingue les statuts ACTIVE (vert) et les autres (gris), propose une recherche et réinitialise les filtres descendants lorsque le parent change. La sélection est conservée dans l’URL. Sans campagne, le choix d’ensemble est désactivé.

`lyads_scoped_dashboard_metrics`, exécutée avec les droits de l’appelant, agrège les décimales en PostgreSQL. KPI, courbes et placements utilisent le niveau compte, campagne ou ensemble correspondant au filtre, jamais un mélange de ces niveaux. La ventilation par campagne utilise la même sélection, et les publicités sont filtrées via leurs ensembles parents. Les répartitions ne sont jamais additionnées aux totaux. Toutes les zones reçoivent le même périmètre ; `lyads_scoped_performance_alerts` restreint les détecteurs aux entités de ce périmètre. Les problèmes techniques de connexion restent applicables au compte. Les recommandations demeurent vides tant que leur moteur ne fournit aucun résultat réel.
...
Les détecteurs de performance utilisent les seuils configurés et les données suffisantes ; leurs résultats sont différés pendant un import actif. Le moteur de recommandations ne fournit pas encore de recommandations : aucune n’est fabriquée. La vue conserve les problèmes techniques et alertes de performance Lyads, et exclut les statuts de diffusion Meta. Les cinq premières publicités suivent le classement décrit ci-dessous. Les listes de campagnes et placements sont limitées à 50 résultats.
...
`lyads_scoped_account_ads` agrège et trie en PostgreSQL, avec RLS, sur le compte, les dates, la campagne et l’ensemble demandés. Campagnes actives et arrêtées sont incluses. L’événement vient de `adset.promoted_object.custom_event_type` ou `custom_conversion_id`, puis des objectifs d’optimisation explicitement reconnus. Un objectif « ventes » seul ne prouve pas un événement achat.

Les achats suivent ROAS décroissant, coût par achat croissant, achats décroissants ; les autres événements suivent coût par résultat croissant, résultats décroissants. L’identifiant tranche les égalités. Chaque événement garde cet ordre ; en vue multi-événements, les positions sont entrelacées (premier de chaque événement, puis deuxième, etc., achats en premier en cas d’égalité de position), sans comparer directement un coût par lead à un coût par achat. Les publicités avec résultats exploitables précèdent celles sans résultats ; celles sans aucune ligne sur la période arrivent en dernier. Cette règle de présentation ne constitue pas un score commun à des objectifs différents.

Aucun seuil de dépense, durée, impressions, clics ou conversions n’exclut une publicité du classement. Les seuils de fiabilité restent uniquement dans les détecteurs d’alertes. Les valeurs de conversion effectivement rapportées sont additionnées sur la période, même si une journée ne comporte pas l’entrée correspondante dans le tableau Meta. Aucun alias n’est additionné. Une métrique absente sur toute la période reste nulle ; une conversion constatée sans valeur monétaire empêche d’en déduire le ROAS. Référence de lecture des tableaux d’actions : [exemple officiel Meta](https://github.com/fbsamples/marketing-api-samples/blob/master/samples/samplecode/ads_reporting.py).
...
La présentation reprend le tableau de la maquette : vignette et nom, dépense, CPA ou CPR selon les événements affichés et ROAS. La colonne État a été supprimée. Le survol ou le clic ouvre le média en superposition. Le clic maintient l’aperçu ouvert ; sa fermeture arrête la vidéo. La couleur du ROAS utilise les cibles enregistrées ; sans cible exploitable, elle reste neutre. Aucune fiabilité ou projection n’est inventée.
...
Validation réelle du 14 septembre : les filtres exposent les 26 campagnes importées (actives et arrêtées). La sélection « Booster Abidjan → Broad Adset CI 2 HF » affiche 32,33 EUR de dépense et quatre publicités, tandis que les niveaux campagne et compte gardent leurs agrégats propres. La première page du compte inclut désormais Nv4 malgré un seul achat, selon son ROAS, et Nv6 conserve les achats rapportés sur les jours disponibles. Les deux migrations de lecture ont été appliquées à Supabase ; aucun changement de campagne Meta n’est effectué par ces filtres.
